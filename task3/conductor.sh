...
run()
{
    ...
    unshare -u -p -n -m -i -f chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c "mount -t proc proc /proc && mount -t sysfs sys /sys && chmod 755 / && $INIT_CMD_ARGS"
}
...
exec()
{
    ...
    nsenter --all --root --wd --target $CONTAINER_INIT_PID $EXEC_CMD_ARGS
